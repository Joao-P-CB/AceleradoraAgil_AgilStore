def ler_numero(flag_type): # Funcao que le entrada do teclado e testa caso o usuario inseriu um numero ou nao
    ...
    if flag_type == 1: # flag_type define qual a mensagem que deve aparecer na tela para pedir para o usuario
        while repetir == 1: # Repete ate que o usuario insira um numero valido
            ...
            try:
                return int(num)
            except ValueError as err: # Solucao adapatada do link: https://pt.stackoverflow.com/questions/288558/como-eu-fa%C3%A7o-prevenir-que-o-input-do-usu%C3%A1rio-n%C3%A3o-resulte-em-um-valueerror-e-ped
...
def busca(j, b_id): # Funcao que busca um parametro dentro da lista, variavel j alinha produtos com o tipo de informacao encontrada em b_id
    encontrou = 0
    i = 0
...
def imprime_produto(resultado): # Imprime os dados de um produto especifico, recebendo uma flag que diz se o produto não existe ou a posição dele
    if resultado == -1:
                ...
                if len(produtos) == 0: # Caso a lista de produtos esteja vazia, não realiza testes, apenas segue o processo de inserção
                    ...
                    if existe == -1: # Se retornar -1 significa que o produto não se encontra na lista, então segue o código
                        ...
                        while teste_continuar != 1 and teste_continuar != 2:
                            ...
                            teste_continuar = int(input("Gostaria de tentar novamente?\n 1) Sim \n 2) Nao \n\nDigite a opcao desejada: "))
                        ...
                        while alterar != 0: # Mantém o usuário nas alterações até que ele deseja sair
                            ...
                            match alterar:
                                case 1:
                                    ...
                                    for i in range(len(produtos)): # Testa se o novo nome já existe dentro da lista, caso sim volta para as opções de alterações
                                        if i != index_troca:
                                ...
                                case 0:
                                    ...
                                    if alterado == 1: # Teste se houve alguma alteração válida
                                        while confirmar == -1:
                                            ...
                                            match conf_alt:
                                                case 1:
                                                    for i in range(len(produto_alt)): # Caso o usuário confirme as alterações, insere todos os valores que não sejam -1
                                                        if produto_alt[i] != -1:
                    ...
                    if remove_id != 0: # Caso o usuário digite um ID que não seja 0 a função continua
                        ...
                        if index_remover == -1: # Se não encontrou o ID o usuário deverá tentar novamente
                            ...
                            while confirmar != 1 and confirmar != 2:
                                confirmar = int(input("\nConfirmar a exclusao do item? \n 1) Sim \n 2) Nao \n Digite a opcao desejada: ")) # Confirma com o usuário sobre a exclusão
